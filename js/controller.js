...
TestApp.controller('homeCtrl', function($scope,$http,$location) {
  ...
  $scope.submit=function()
  {
      ...
      if(!angular.isDefined($scope.searchbox) || $scope.searchbox=== '') 
      {
       ...
       $http.get("http://localhost:6200/blah/"+id).then(function(response){
...
angular.module('TestApp.controllers', ['ui.bootstrap']).controller('testController',  ['$scope', '$http', '$uibModal', '$routeParams' ,function($scope, $http, $uibModal,$routeParams) {
    $scope.loading = false;
    var modalInstance = null;
    console.log($routeParams.username);
    $scope.getData = function() {
        $scope.loading = true;
        $http.get("http://localhost:6200/blah")
        .then(function(response){
            $scope.names = response.data;
            $scope.demo = $routeParams.username;
            $scope.loading = false;
        });
    }
